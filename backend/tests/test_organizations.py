...
def members_url(slug):
    return f"{ORGS_URL}{slug}/members/"


def member_url(slug, user_id):
    return f"{ORGS_URL}{slug}/members/{user_id}/"
...
class TestOrganizationDetail:
    def test_get_organization(self, authenticated_client, organization):
        response = authenticated_client.get(org_url(organization.slug))
        assert response.status_code == status.HTTP_200_OK
...
class TestOrganizationMembers:
    ...
    def test_add_member(self, authenticated_client, organization):
        ...
        response = authenticated_client.post(
            members_url(organization.slug), data, format="json"
        )
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_remove_member(self, authenticated_client, organization):
        ...
        response = authenticated_client.delete(
            member_url(organization.slug, member_user.id)
        )
        assert response.status_code == status.HTTP_204_NO_CONTENT
