...
def list_sub_agents_for_user(user):
    ...
    return SubAgent.objects.filter(Q(organization_id__in=user_org_ids) | Q(organization__isnull=True)).order_by("name")


def list_sub_agents_for_organization(organization):
    return SubAgent.objects.filter(Q(organization=organization) | Q(organization__isnull=True)).order_by("name")
