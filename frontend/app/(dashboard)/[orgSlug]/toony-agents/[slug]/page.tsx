...
export default function ToonyAgentDetailPage() {
  ...
  const fetchAgent = useCallback(async () => {
    try {
      const data = await getToonyAgent(orgSlug, slug);
      setAgent(data);
    } finally {
      setIsLoading(false);
    }
  ...
  const fetchTasks = useCallback(async () => {
    try {
      setTasks((await listAgentTasks(orgSlug, slug)).results);
    } catch {
  ...
  function handleTaskCreated(taskId: string) {
    router.push(`/${orgSlug}/toony-agents/${slug}/tasks/${taskId}`);
  }
  ...
  return (
    <div>
      {/* Back link */}
      <button
        onClick={() => router.push(`/${orgSlug}/toony-agents`)}
            ...
            <table className="min-w-full divide-y divide-slate-800/60">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {tasks.map((task) => (
                  <tr
                    key={task.id}
                    onClick={() => router.push(`/${orgSlug}/toony-agents/${slug}/tasks/${task.id}`)}
