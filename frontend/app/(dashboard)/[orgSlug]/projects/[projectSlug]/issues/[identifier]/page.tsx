...
export default function IssueDetailPage() {
  const params = useParams<{ orgSlug: string; projectSlug: string; identifier: string }>();
  ...
  useEffect(() => {
    (async () => {
      try {
        const [proj, membersRes, milestonesRes, cyclesRes, labelsRes] = await Promise.all([
  ...
  async function saveTitle() {
    ...
    try {
      await updateIssue(orgSlug, projectSlug, identifier, { title: trimmed });
      await fetchIssue();
      setEditingTitle(false);
    } catch {
      // revert on error
